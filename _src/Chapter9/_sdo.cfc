<cfcomponent output="false">
	...
	<cffunction name="changeRow" access="public" output="false">
		<cfargument name="original" type="struct">
		<cfargument name="change" type="struct">
		
		<cfif listLen(structKeyList(arguments.change))>
			<cfquery datasource="#variables.dsn#" name="qryUpdate">
			UPDATE #variables.table#
			SET<cfloop list="#arguments.fields#" index="iField"><cfif iField NEQ variables.pKeyField AND structKeyExists(arguments.change,iField)>
				#seperator##iField# = <cfqueryparam value="#arguments.change[iField]#"><cfset seperator = ", "></cfif></cfloop>
			WHERE
				#variables.pKeyField# = <cfqueryparam value="#arguments.original[variables.pKeyField]#">
			</cfquery>
		</cfif>
	...
	<cffunction name="getRecordset" access="public" output="false">
		...
		WHERE 
			1=1 
			AND
			#arguments.where#</cfif><cfif arguments.order NEQ "" AND trim(arguments.order) NEQ "ASC">
		ORDER BY
	...
	<cffunction name="getGridRecordSet" access="remote" returntype="struct">
		...
		<cfscript>
		var recordset = "";
		var sort="#arguments.gridSortColumn# #arguments.gridSortDirection#";
		
		recordset = getRecordSet(order:"#trim(sort)#");
		
		return QueryConvertForGrid(recordset,
